...
# Application Load Balancer (ALB) routing traffic to the app, and Auto Scaling enabled for the ECS Service.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
module "ecs_cluster" {
  ...
  # this shows an example of how to use ECS capacity providers
...
data "aws_iam_policy_document" "access_test_s3_bucket" {
  statement {
    ...
    resources = [
      "${aws_s3_bucket.s3_test_bucket.arn}/${var.s3_test_file_name}",
    ]
  }
...
module "ecs_service" {
  ...
  capacity_provider_strategy = [
    for name in module.ecs_cluster.ecs_cluster_capacity_provider_names :
    {
...
# ---------------------------------------------------------------------------------------------------------------------
# DEFINE SOME AUTO SCALING POLICIES FOR OUR ECS SERVICE
# ---------------------------------------------------------------------------------------------------------------------
...
# Create a CloudWatch Alarm to trigger our Auto Scaling Policy when CPU Utilization is high.
...
# Create a CloudWatch Alarm to trigger our Auto Scaling Policy when CPU Utilization is low.
