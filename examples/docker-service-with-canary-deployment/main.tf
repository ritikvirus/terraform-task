# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# DEPLOY A DOCKER APP WITH AN ELASTIC LOAD BALANCER IN FRONT OF IT AND SUPPORT FOR CANARY DEPLOYMENT
# These templates show an example of how to run a Docker app on top of Amazon's EC2 Container Service (ECS) with an
# Elastic Load Balancer (ELB) routing traffic to the app. It supports canary deployment to test out a single instance
# of a new Docker container version before deploying that version across the whole cluster.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
locals {
  container_definition = templatefile(
    "${path.module}/containers/container-definition.json",
    {
      ...
      awslogs_group       = aws_cloudwatch_log_group.ecs_task.name
...
# ---------------------------------------------------------------------------------------------------------------------
# CREATE AN ECS TASK TO RUN A CANARY VERSION OF MY DOCKER CONTAINER
# This canary runs a different version of our Docker container and returns different server text when you curl it
# ---------------------------------------------------------------------------------------------------------------------

# This local defines the Docker containers we want to run in our canary ECS Task
locals {
  canary_container_definition = templatefile(
    "${path.module}/containers/container-definition.json",
    {
      container_name = var.service_name
      # For this example, we run the Docker container defined under examples/example-docker-image.
      ...
      awslogs_group       = aws_cloudwatch_log_group.ecs_task.name
      awslogs_region      = var.aws_region
      awslogs_prefix      = "${var.service_name}-canary"
    },
  )
}
...
module "ecs_service" {
  ...
  # Set deployment parameters so that we update 1 task at a time.
